...
const storedButtonPositionPref = prefPerPage<any>('open_button_position', { x: 0, y: 0 })

export const OpenButton: React.FC<any> = () => {

    const draggableRef = useDraggable(safeDisplayPosition(storedButtonPositionPref.get()), (x: number, y: number) => {
        storedButtonPositionPref.set({ x, y, })
    });

    return (
        <div ref={draggableRef as any} style={{ position: 'fixed', cursor: 'move' }}>
            <Button variant="outline" className="ab-rounded-full ab-ftr-bg ab-w-[40px] ab-h-[40px] !ab-p-0 !ab-m-0">
