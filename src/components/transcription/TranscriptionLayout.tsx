...
import { createRef, useCallback, useEffect, useRef, useState, type SyntheticEvent } from "react"
import { MarkdownEditor } from "../MarkdownEditor"
import AudioPlayer from "../../ui/audio-player"
import { audioBufferToWav, blobToDataUrl, getAudioFileAsAudioBuffer, processAudioBufferWithBandpass, sliceAudioBufferAtPauses } from "../../lib/audio-dsp"
...
async function transcribeInWorker(blob: Blob) {
    ...
    return new Promise((resolve, reject) => {
        ...
        (async() => {
            chrome.runtime.sendMessage({action: "transcribe", text: JSON.stringify({ blobDataUrl: await blobToDataUrl(blob) })}, (response) => {
...
export const TranscriptionLayout = () => {
    ...
    const [audioBufferSlices, setAudioBufferSlices] = useState<Array<AudioBuffer>>([])
    const [audioBufferBlobs, setAudioBufferBlobs] = useState<Array<{ duration: number, blob: Blob}>>([])
    const [editorValue, setEditorValue] = useState<string>("")
    const [selectedMediaElement, setSelectedMediaElement] = useState<HTMLMediaElement | null>(null)
    ...
    const onFindVisibleMediaElements = useCallback(() => {
        ...
        const mediaElements = mediaElementSelectors.map(selector => document.querySelector(selector) as HTMLMediaElement | null)
        const visibleMediaElements = filterForVisibleElements(mediaElements).filter((el) => el !== null) as HTMLMediaElement[]
        ...
        (async() => {
            if (audioFile && audioContext) {
                ...
                if (audioPlayerStatusRef.current) {
                    audioPlayerStatusRef.current.innerHTML = "Applying bandpass filter..."
                }

                setAudioBuffer(await processAudioBufferWithBandpass(await getAudioFileAsAudioBuffer(audioFile, audioContext)))
    ...
    useEffect(() => {
        ...
        if (videoEl$) {
            ...
            clonedEl.classList.add('ab-w-full', 'ab-h-full', 'ab-relative', 'ab-block')
            mediaElContainerRef.current?.appendChild(clonedEl)
        }

    }, [videoEl$, mediaElContainerRef])
    
    const onTranscribeSliceClick = useCallback((blob: Blob, index: number, elapsedMins: number, elapsedSecs: number) => {
        return async() => {
            console.log('transcribe slice', blob)
            const transcription = (await transcribeInWorker(blob)) as { text: string }
            console.log('done transcribe slice', transcription)

            setEditorValue(`${editorValue}\n\n${index}. ${elapsedMins.toFixed(0)}:${elapsedSecs.toFixed(0)}\n${transcription.text}`)
        }
    }, [editorValue])


    const onSelectMediaElement = useCallback((e: any) => {
    ...
    const onTranscribeMediaElement = useCallback(() => {
        if (selectedMediaElement) {
            ...
            if ('captureStream' in selectedMediaElement) {
                const stream = (selectedMediaElement as HTMLMediaElementWithCaptureStream).captureStream();
    ...
    return (
        <ResizablePanelGroup direction="vertical">
                        ...
                        <Card className="ab-w-full ab-h-full ab-flex ab-justify-between ab-items-stretch ab-flex-col">
                            <div>
                                <CardHeader>
                                    ...
                                    <CardDescription>In diesem Modus wird Dein Browser das Transkodieren in Echtzeit starten. Es werden keine Daten vorab auf Server, nur an die KI gesendet hochgeladen.</CardDescription>
                                </CardHeader>
                                <CardContent>
                                    <form>
                                    <div className="ab-grid ab-w-full ab-items-center ab-gap-2">
                                        ...
                                        <div className="ab-flex ab-w-full ab-items-center ab-space-y-1.5 ab-space-x-2">
                                            <Input type="file"  onClick={(event: any)=> { 
                                                    event.target.value = null
                                                }} onChange={onTranscribe} placeholder="Select audio or video file..." />
                                        </div>
                                    </div>
                                    </form>
                                </CardContent>
                        ...
                        <Card className="ab-w-full ab-h-full ab-flex ab-justify-between ab-items-stretch ab-flex-col">
                            <div>
                                <CardHeader>
                                    ...
                                    <CardDescription>Wähle ein Video oder Audio-Element aus, das auf dieser Website erkannt wurde, um es in Echtzeit zu transkribieren.</CardDescription>
                                </CardHeader>
                                                ...
                                                <select onSelect={onSelectMediaElement}>
                                                    {
                                                        mediaElements.map((el, index) => {
                                                            return (
                                                                <option key={index+el.tagName}>{index+1}: {el.tagName} (Länge: {formatDuration(el.duration)})</option>
                                                            )
                                                        })
                                                    }
                                                </select>
                            ...
                            <div>
                                <CardFooter className="ab-flex ab-justify-between">
                                    ...
                                    <Button onClick={onTranscribeMediaElement}>Transkribieren</Button>
                                </CardFooter>
                            </div>
                        </Card>
                    </ResizablePanel>
            ...
            <ResizablePanel defaultSize={50} minSize={40} className="!ab-overflow-y-scroll ab-h-full ab-flex ab-flex-col ab-w-full">
                    ...
                    <ResizablePanelGroup direction="horizontal">
                        ...
                        <ResizablePanel defaultSize={25} minSize={20}>  
                            ...
                            <div className="ab-flex ab-flex-row ab-ml-1 ab-sticky ab-top-0 ab-z-30  ab-justify-between">
                                <div ref={audioPlayerContainerRef} className="ab-h-[100px] ab-rounded-md ab-overflow-scroll ab-w-[100%]">
                                    {(audioBuffer || audioBlob) && (<>
                                        Player:
                                        <AudioPlayer className="w-[100%]" audioBlob={audioBuffer ? audioBufferToWav(audioBuffer) : audioBlob} />
                                        </>
                                    )}
                                    <div ref={audioPlayerStatusRef} className="ab-flex ab-justify-center ab-items-center ab-h-full">
                                    </div>
                                </div>

                                {!(audioBuffer || audioBlob) && (
                                    <p className="ab-p-2">Der Player wird angezeigt, sobald die Transkription gestartet wurde.</p>
                                )}
                            ...
                            <div className="ab-flex ab-h-[100%] ab-w-[100%] ab-overflow-auto ab-flex-col">
                                {audioBufferBlobs.map((blob, index) => {
                                    ...
                                    const elapsedTime = audioBufferBlobs.reduce((acc, blob, i) => {
                                        if (i <= index) {
                                    ...
                                    return (
                                        <div key={index} className="ab-flex ab-flex-col ab-m-2 ab-mb-4">
                                            <div className="ab-flex ab-flex-row">
                                                <Badge variant="outline">{index + 1}</Badge>
                                                <span className="ab-font-mono">
                                                    {elapsedMins.toFixed(0)}:{elapsedSecs.toFixed(0)}: {blob.duration.toFixed(2)} Sek., {sizeInMiB.toFixed(2)} MiB
                                                </span>
                                            </div>
                                            <AudioPlayer audioBlob={blob.blob} />
                                            <button onClick={onTranscribeSliceClick(blob.blob, index + 1, elapsedMins, elapsedSecs)}>Transcribe</button>
                                        </div>
                                    );
                                })}
                                {audioBufferBlobs.length === 0 && (
                                    <p className="ab-p-2">Dieser Bereich wird mit vorverarbeiteten Audio-Chunks befüllt, sobald die Vor-Verarbeitung der Transkription durchgeführt wurde.</p>
                                )}
                            </div>
                        </ResizablePanel>
                        ...
                        <ResizablePanel defaultSize={75} minSize={20}>  
                            ...
                            <div className="ab-flex ab-h-full ab-items-stretch ab-w-full ab-justify-stretch">
                                <MarkdownEditor 
                                    ...
                                    placeholder={"Das Ergebnis wird durch jeden Klick auf 'Transkribieren' pro Audio-Chunk erweitert."}
