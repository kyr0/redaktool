...
import ONNX from 'onnxruntime-web/webgl';
const { executionProviders, InferenceSession, Tensor: ONNXTensor, env } = ONNX;
...
async function sessionRun(session, inputs) {
    ...
    try {
        ...
        checkedInputs.input_ids.location = "cpu";
        checkedInputs.attention_mask.location = "cpu";
        checkedInputs.token_type_ids.location = "cpu";
...
async function encoderForward(self, model_inputs) {
    ...
    if (model_inputs.input_ids.dataLocation === "cpu") {
        model_inputs.input_ids.data = model_inputs.input_ids.cpuData.slice();
    }

    if (model_inputs.attention_mask.dataLocation === "cpu") {
        model_inputs.attention_mask.data = model_inputs.attention_mask.cpuData.slice();
    }
    ...
    console.log("encoderFeeds", encoderFeeds, model_inputs);
    if (self.session.inputNames.includes('token_type_ids') && !encoderFeeds.token_type_ids) {
        ...
        encoderFeeds.token_type_ids = new Tensor(
            'int64',
            new BigInt64Array(model_inputs.input_ids.dataLocation === "cpu" ? 
                model_inputs.input_ids.cpuData.length : encoderFeeds.input_ids.data.length),
